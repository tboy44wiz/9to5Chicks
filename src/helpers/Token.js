...
class Token {
  ...
  */
    ...
    return async (req, res, next) => {
      ...
      try {
        ...
        if (!user || user.role !== role) {
          return res.status(401).json(
            new Response(false, 401, `Unauthorized, Only ${role}s can access this resource`)
          );
        }
        req.payload = decoded;
        return next();
  ...
  static verifyInstructorToken(role) {
    return async (req, res, next) => {
      const token = req.headers.authorization
      || req.headers['x-access-token'] || req.query.token || req.body.token;
      if (!token) {
      ...
      try {
        ...
        if (!user || user.role !== role) {
          return res.status(401).json(
            new Response(false, 401, `Only ${role}s can access this resource`)
          );
        }
        req.payload = decoded;
        return next();
